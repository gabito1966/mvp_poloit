        ...
        <div className="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
            <table className="min-w-full border-collapse rounded-lg">
                <thead className="bg-gray-100 dark:bg-gray-600 ">
                    <tr>
                        <th className="text-left p-3 font-semibold text-xs sm:text-sm ">Apellido y Nombre</th>
                        <th className="text-left p-3 font-semibold text-xs sm:text-sm">Email</th>
                        <th className="text-left p-3 font-semibold text-xs sm:text-sm">Teléfono</th>
                        <th className="text-left p-3 font-semibold text-xs sm:text-sm">Tecnología</th>
                    </tr>
                </thead>
            ...
            <table className="min-w-full border-collapse">
                <thead className="bg-gray-100 dark:bg-gray-600">
                    <tr>
                        ...
                        <th className="text-left p-3 font-semibold text-xs sm:text-sm">Teléfono</th>
                        <th className="text-left p-3 font-semibold text-xs sm:text-sm">Tecnología</th>
                        <th className="text-left p-3 font-semibold text-xs sm:text-sm">Estado</th>
                    </tr>
                </thead>
                <tbody className="bg-white dark:bg-gray-800">
                    {equipo.nombres_estudiantes.map((nombre, index) => (
                        <tr key={index} className="border-b border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                            <td className="p-3 text-xs sm:text-sm" title={`${equipo.apellidos_estudiantes[index]}, ${nombre}`}>{`${equipo.apellidos_estudiantes[index]}, ${nombre}`}</td>
                            <td className="p-3 text-xs sm:text-sm" title={`${equipo.emails_estudiantes[index]}`}>{equipo.emails_estudiantes[index]}</td>
                            <td className="p-3 text-xs sm:text-sm" title={`${equipo.telefonos_estudiantes[index]}`}>{equipo.telefonos_estudiantes[index]}</td>
                            <td className="p-3 text-xs sm:text-sm" title={`${equipo.tecnologias[index]}`}>
                                <span className="inline-block bg-gray-200 dark:bg-gray-700 rounded-full px-2 py-1 text-xs font-semibold">
                                    {equipo.tecnologias[index].length < 6 ? equipo.tecnologias[index] : equipo.tecnologias[index].slice(0,6)+"..."}
                                </span>
                            </td>
                            <td className="p-3 text-xs sm:text-sm" title={`${equipo.estados_estudiantes[index]}`}>
                            <div className="flex items-center">
                                        {getStatusIcon(equipo.estados_estudiantes[index])}
                                        <span className={`ml-2 px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(equipo.estados_estudiantes[index])}`}>
...
export default async function EquipoCard({
    ...
    if (!equipo) {
        return <p className="text-center text-red-500 font-semibold">Error al cargar el equipo.</p>; 
    }
            ...
            <div className="bg-white dark:bg-slate-700 rounded-xl shadow-lg overflow-hidden">
                ...
                <div className=" p-4 sm:p-6 space-y-6 sm:space-y-8">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 bg-gray-100 dark:bg-slate-800 rounded-lg p-4">
                        <div>
                            <h4 className="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">Nombre:</h4>
                            <p className="text-base sm:text-lg font-semibold mt-1">{equipo.nombre}</p>
                        </div>
                        <div>
                            <h4 className="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">Integrantes:</h4>
                            <p className="text-base sm:text-lg font-semibold mt-1">{equipo.tamano}</p>
                        </div>
                        <div>
                            <h4 className="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">Fecha Inicio:</h4>
                            <p className="text-base sm:text-lg font-semibold mt-1">{fecha_inicial}</p>
                        </div>
                        <div>
                            <h4 className="text-xs sm:text-sm font-medium text-gray-500 dark:text-gray-400">Finaliza:</h4>
                            <p className="text-base sm:text-lg font-semibold mt-1">{fecha_final}</p>
                        </div>
                    </div>
                    <div>
                        <h2 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4">Mentores</h2>
                        <MentorTable mentores={mentores} />
                    </div>
                    <div>
                        <h2 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4">Integrantes</h2>
