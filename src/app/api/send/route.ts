...
export async function POST(request: Request) {
  ...
  try {
    ...
    resultGrupos.map(async (e,i)=>{
      ...
      const emailBody = generateHTMLString(mensaje, validateJWT.nombre as string ,validateJWT.apellido as string,e );
      ...
      const { data, error } = await emailService.sendEmail({
        from: "Polo-IT ",
        to: e.emails_estudiantes, //todos de ese grupo si es true, si es false individual a cada uno
